...
class Card extends Component {
  ...
  flipCard = () => {
    if (this.value >= 90) {
      ...
      this.setState({faceCard: 'Question'})
    }
  }

  render() {
    const {card} = this.props;
    const {faceCard} = this.state
    ...
    return (
      <View style={[styles.container]}>
        ...
        <TextButton style={{color: red, fontSize: 18}} onPress={() => this.flipCard()}>{faceCard}</TextButton>
      </View>
    )
  }
}
